...
figure1alow <- clean_1a %>%
  ...
  mutate(total.n = n(), total.tradeoff = sum(tradeoff), prop.tradeoff = sum(tradeoff)/n(), total.location = sum(location), prop.location = sum(location)/n(), SE.tradeoff = sqrt(prop.tradeoff * (1 - prop.tradeoff)/total.n), CI.tradeoff = qnorm(.975) * SE.tradeoff, CI.tradeoff.high = (CI.tradeoff / 2), CI.tradeoff.low = (CI.tradeoff / 2), SE.location = sqrt(prop.location * (1 - prop.location)/total.n), CI.location = qnorm(.975) * SE.location, CI.location.high = (CI.location / 2), CI.location.low = (CI.location / 2)) %>%
...
CI.high = c(figure1alow$CI.tradeoff.high, figure1alow$CI.location.high, figure1ahigh$CI.tradeoff.high, figure1ahigh$CI.location.high)
CI.low = c(figure1alow$CI.tradeoff.low, figure1alow$CI.location.low, figure1ahigh$CI.tradeoff.low, figure1ahigh$CI.location.low)
fig1 <- data.frame(title, values, income, CI.high, CI.low)
#figure 1a data frame

figure1a <- fig1 %>%
  ggplot(aes(x = title, y = values, fill = income)) + geom_bar(stat = "identity", position = position_dodge(), colour = "black") + ylab("Proportion Endorsing Reason") + xlab("Reason") + geom_errorbar(aes(ymin = values - CI.low, ymax = values + CI.high), width = 0.2, position = position_dodge(0.9)) + theme_bw() + scale_fill_grey(start = .85, end = .3) + ggtitle("Figure 1a - the proportion of lower- and higher-income participants\nciting trade-off and location considerations in Study 1a")
#produces figure 1a
