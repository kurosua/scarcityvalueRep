...
<br> <br>
*Update 4/25/15 - The unmatched beta coeffients are due to the publication reporting standardized coefficients, and R function lm reporting unstandardized beta coeffients*
...
> The interaction was also significant when income was treated as a continuous variable beta = 2.27, t(145) = 2.17, p < 0.05. *(unmatched beta coefficient)*

```{r echo = FALSE}
lm(beer ~ resort * logses, data = clean_1b) %>%
  tidy() %>%
  filter(term == "resort:logses") %>%
  mutate(beta = round(estimate, digit = 2),
         p = round(p.value, digit = 4))  %>%  
        select(term, beta, p)
...
> The interaction was also significant when income was treated continuously, beta = 1.65, t(574) = 3.27, p < 0.01. *(unmatched beta coefficient)*

```{r echo = FALSE}
lm(pay ~ resort * logses, data = clean_1c) %>%
  tidy() %>%
  filter(term == "resort:logses") %>%
  mutate(beta = round(estimate, digit = 2),
         p = round(p.value, digit = 4))  %>%  
...
> This interaction was marginally significant when income was treated continuously, beta = 0.50, t(1894) = 1.85, p < 0.07. *(unmatched beta coefficient)*

```{r echo = FALSE}
lm(wtp ~ resort * logses, data = clean_1d) %>%
  tidy() %>%
  filter(term == "resort:logses") %>%
  mutate(beta = round(estimate, digit = 2),
         p = round(p.value, digit = 4)) %>%
...
> The interaction was also significant when income was treated continuously, β = 1.13, t(499) = 2.21, p < .05. *(unmatched beta coefficient)*

```{r echo = FALSE}
# again, we find a larger beta coeffieicnt, but matching t value and p value.
lm(judge ~ logses * smallloss, data = df3_spss) %>%
  tidy() %>%
  filter(term == "logses:smallloss") %>%
  mutate(beta = round(estimate, digit = 2),
         p = round(p.value, digit = 4)) %>%
  select(term, beta, p)
```
...
> The interaction was also significant when income was treated as a continuous variable, β = 2.43, t(255) = 3.39, p < .01. *(unmatched beta coefficient)*

*Note: beta estimate and t value are negative, but are reported as positive*
```{r echo = FALSE}

lm(expense ~ largeacc * logses, data = df4_spss) %>%
  tidy() %>%
  filter(term == "largeacc:logses") %>%
  mutate(beta = round(estimate, digit = 2),
         p = round(p.value, digit = 4)) %>%
  select(term, beta, p)
...
> and the interaction was also found when dieting behavior was treated continuously, β = 0.43, t(230) = 2.11, p < .05. *(unmatched beta coefficient)*

```{r echo = FALSE}

lm(fattening ~ log * week, data = df5_spss) %>%
  tidy() %>%
  filter(term == "log:week") %>%
  mutate(beta = round(estimate, digit = 2),
         p = round(p.value, digit = 4)) %>%
  select(term, beta, p)
