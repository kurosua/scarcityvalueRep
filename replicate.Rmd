...
bin = c(1, 2, 3, 4)
title = factor(c("tradeoff", "location"), levels = c("tradeoff", "location"))
...
fig1 <- data.frame(bin, title, values, income)

fig1 <- fig1 %>%
  ggplot(aes(x = title, y = values, fill = income)) + geom_bar(stat = "identity", position = position_dodge(), colour = "black") +  ylab("Proportion Endorsing Reason") + xlab("Reason")
