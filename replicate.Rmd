...
```
<br> <br> <br>
*note: beta coefficients for linear regressions calculate consistantly larger than published, even though the t value and p values are as published.  This occurs several times throughout.*
...
summary(lm(judge ~ logses * smallloss, data = df3_spss))
```
...
summary(lm(expense ~ largeacc * logses, data = df4_spss))
...
summary(lm(fattening ~ log * week, data = df5_spss))
