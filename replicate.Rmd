...
Briefly: this paper was about the effects of scarcity (e.g. financial, caloric, etc) interacts other contextual psychological effects. 
...
# Set values for figure
title <- factor(c("trade-offs", "location"), 
                levels = c("trade-offs", "location"))
values <- c(figure1alow$prop.tradeoff, 
            figure1alow$prop.location, 
            figure1ahigh$prop.tradeoff, 
            figure1ahigh$prop.location)
income <- c("lower income", 
            "lower income", "higher income", 
            "higher income")
CIs <- c(figure1alow$CI.tradeoff, 
         figure1alow$CI.location, 
         figure1ahigh$CI.tradeoff, 
         figure1ahigh$CI.location)
...
clean_1b %>%
  ...
  summarise(mean.beer = mean(beer), 
            CI.low = mean(beer) -  (1.96 * Sem(beer)), 
            CI.high = mean(beer) + (1.96 * Sem(beer))) %>%
  round(digits = 2)
# 1.96 * standard error gives 95% CI in one direction

# Generates mean and CI for high income (noted by "split") participants in the resort scenario - to use in figure 1b
...
```{r echo = FALSE}
# this calculates low income WTP beer price at grocery store (resort=0):
clean_1b %>%
...
```{r echo = FALSE}
# ANOVA
summary(aov(beer ~ resort * split, data = clean_1b))

# partial eta squared
etaSquared(aov(beer ~ resort * split, data = clean_1b)) %>%
  .["resort:split", "eta.sq.part"] %>%
  round(digit = 3) %>%
  cat("Grocery/Resort & Low/High-Income\nPartial Eta Squared:",.)
```
...
raw_1c_spss %>%
  summarise(median.household.income = median(inc2), 
            median.household.size = median(house))
...
clean_1c %>%
  filter(split == 1, resort == 0) %>%
  summarise(mean.beer = mean(pay), 
            ci.low = mean(pay) -  (1.96 * Sem(pay)), 
            ci.high = mean(pay) + (1.96 * Sem(pay))) %>%
  round(digits = 2)
...
# partial eta squared
etaSquared(aov(pay ~ resort * split, data = clean_1c)) %>%
  .["resort:split", "eta.sq.part"] %>%
  round(digit = 3) %>%
  cat("Grocery/Resort & Low/High-Income\nPartial Eta Squared:",.)
```
...
values <- c(clean_1c_low_grocer$mean.pay, 
            ...
            clean_1c_low_resort$mean.pay, clean_1c_high_resort$mean.pay)
location <- c("Grocer", "Grocer", "Resort", "Resort")
CIs <- c(clean_1c_low_grocer$CI, 
         clean_1c_high_grocer$CI, clean_1c_low_resort$CI, 
...
clean_1d %>%
  filter(split == 1, resort == 1) %>%
  summarise(mean.wtp = mean(wtp), 
            CI.low = mean(wtp) -  (1.96 * Sem(wtp)), 
            CI.high = mean(wtp) + (1.96 * Sem(wtp))) %>%
  round(digits = 2)
...
# partial eta squared
etaSquared(aov(wtp ~ resort * split, data = clean_1d)) %>%
  .["resort:split", "eta.sq.part"] %>%
  round(digit = 3) %>%
  cat("Grocery/Resort & Low/High-Income\nPartial Eta Squared:",.)
```
...
logit3logses <- glm(context ~ lgses, 
                    data = clean_2a, 
                    family = "binomial")

wald.test(b = coef(logit3logses),
          Sigma = vcov(logit3logses),
          Terms = 2)$result$chi2[1] %>% 
  round(digits = 3)
```
...
logit4logses <- glm(tradeoff ~ lgses, 
                    data = clean_2a, 
                    family = "binomial")

wald.test(b = coef(logit4logses), 
          Sigma = vcov(logit4logses), 
          Terms = 2)$result$chi2[1] %>% 
  round(digits = 3)
```
...
raw_2b_spss %>%
  summarise(medianHouseholdIncome = median(inc2),
            medianHouseholdSize = median(house))
```
...
clean_2b %>%
  ...
  summarise(threehundredGo = (sum(go) / n()) * 100)

clean_2b %>%
  filter(split == 1, cond == 1) %>%
  summarise(fivehundredGo = (sum(go) / n()) * 100)

clean_2b %>%
  filter(split == 1, cond == 2) %>%
  summarise(onethousandGo = (sum(go) / n()) * 100)
```
...
clean_2b_high_300 <- clean_2b %>%
  filter(split == 1, cond == 0) %>%
  summarise(threehundredGo = (sum(go) / n()), 
            CI = (1.96 * Sem(go))) %>%
  round(digits = 2)

clean_2b_high_500 <- clean_2b %>%
  filter(split == 1, cond == 1) %>%
  summarise(fivehundredGo = (sum(go) / n()), 
            CI = (1.96 * Sem(go))) %>%
  round(digits = 2)

clean_2b_high_1000 <- clean_2b %>%
  filter(split == 1, cond == 2) %>%
  summarise(onethousandGo = (sum(go) / n()), 
            CI = (1.96 * Sem(go))) %>%
  round(digits = 2)
```
...
title <- factor(c("Lower Income", "Higher Income"), 
                levels = c("Lower Income", "Higher Income"))
values <- c(clean_2b_low_300$threehundred.go, 
            clean_2b_high_300$threehundred.go, 
            clean_2b_low_500$fivehundred.go, 
            clean_2b_high_500$fivehundred.go, 
            clean_2b_low_1000$onethousand.go, 
            clean_2b_high_1000$onethousand.go)
tablet_cost <- c("$300", "$300", "$500", "$500", "$1,000", "$1,000")
CIs <- c(clean_2b_low_300$CI, 
         clean_2b_high_300$CI, 
         clean_2b_low_500$CI, 
         clean_2b_high_500$CI, 
         clean_2b_low_1000$CI, 
         clean_2b_high_1000$CI)

# figure 2b data frame
fig2b.table <- data.frame(title, values, tablet_cost, CIs)
fig2b.table$tablet_cost <- factor(fig2b.table$tablet_cost, 
                                  levels = c("$300", "$500", "$1,000"))
figure2b <- fig2b.table %>%
  ...
  geom_bar(stat = "identity", 
           position = position_dodge(), 
           colour = "black") + 
  ylab("proportion willing to travel") + 
  xlab(NULL) + 
  geom_errorbar(aes(ymin = values - CIs, 
                    ymax = values + CIs), 
                width = 0.2, 
                position = position_dodge(0.9)) + 
  ggtitle("Figure 2b - Proportion of participants willing to travel\nfor a discounted tablet computer in Study 2b\n(error bars represent 95% confidence interval)") + 
  theme_bw() + 
  theme(panel.border = element_blank(), 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
...
logit5logses <- glm(go ~ split * cond, 
                    data = clean_2b, 
                    family = "binomial")

# Wald test
wald.test(b = coef(logit5logses), 
          Sigma = vcov(logit5logses), 
          Terms = 4)$result$chi2[1] %>% 
  round(digits = 3)
```
...
logit6logses <- glm(go ~ logses * cond, 
                    data = clean_2b, 
                    family = "binomial")

# Wald test
wald.test(b = coef(logit6logses), 
          Sigma = vcov(logit6logses), 
          Terms = 4)$result$chi2[1] %>% 
  round(digits = 3)
```
...
title <- factor(c("Lower Income", "Higher Income"), 
                levels = c("Lower Income", "Higher Income"))
values <- c(clean_2c_low_300$threehundred.go, 
            clean_2c_high_300$threehundred.go, 
            clean_2c_low_500$fivehundred.go, 
            clean_2c_high_500$fivehundred.go, 
            clean_2c_low_1000$onethousand.go, 
            clean_2c_high_1000$onethousand.go)
tablet_cost <- c("$300", "$300", "$500", "$500", "$1,000", "$1,000")
CIs <- c(clean_2c_low_300$CI, 
         clean_2c_high_300$CI, 
         clean_2c_low_500$CI, 
         clean_2c_high_500$CI, 
         clean_2c_low_1000$CI, 
         clean_2c_high_1000$CI)

# Figure 2b data frame
fig2c.table <- data.frame(title, values, tablet_cost, CIs)
fig2c.table$tablet_cost <- factor(fig2c.table$tablet_cost, 
                                  levels = c("$300", "$500", "$1,000"))
figure2c <- fig2c.table %>%
  ...
  geom_errorbar(aes(ymin = values - CIs, 
                ...
                width = 0.2, 
                position = position_dodge(0.9)) + 
  ggtitle("Figure 2c - Proportion of participants willing to travel\nfor a discounted tablet computer in Study 2c\n(error bars represent 95% confidence interval)") + 
  theme_bw() + 
  theme(panel.border = element_blank(), 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
...
logit7logses <- glm(go ~ logses * webercond, 
                    data = clean_2c, 
                    family = "binomial")

# Wald test
wald.test(b = coef(logit7logses), 
          Sigma = vcov(logit7logses), 
          Terms = 4)$result$chi2[1] %>% 
  round(digits = 3)
```
...
df2d_1 %>%
  ...
  summarise(highPropLarge = (sum(go) / n()) * 100) %>%
  round(digit = 0)

# high-income & proprotionately smaller discount
df2d_1 %>%
  filter(split == 1, lgdiscount == 1) %>%
  summarise(highPropSmall = (sum(go) / n()) * 100) %>%
  round(digit = 0)
```
...
df2d_1 %>%
  ...
  summarise(higgPropLarge = (sum(go) / n()) * 100) %>%
  round(digit = 0)
...
```{r echo = FALSE}
temp_model <- glm(go ~ split*lgdiscount*lgsavings, data = df2d_1, family = "binomial")
...
# wald test
print(wald.test(b = coef(temp_model), 
                Sigma = vcov(temp_model), 
...
```{r echo = FALSE}
temp_model <- glm(go ~ split * lgdiscount * lgsavings, 
                  data = df2d_1, 
                  family = "binomial")
...
# wald test
wald.test(b = coef(temp_model), 
          Sigma = vcov(temp_model), 
          Terms = 8)$result$chi2[1] %>% 
  round(digits = 3)
```
...
```{r echo = FALSE}
# GLM
temp_model <- glm(go ~ lgses * lgdiscount * lgsavings, 
                  data = df2d_spss, 
                  family = "binomial")
...
df3_spss %>%
  ...
  summarise(meanJudgeHighIncomeLossGamble = mean(judge), 
            CI.low = mean(judge) - (1.96 * Sem(judge)), 
            CI.high = mean(judge) + (1.96 * Sem(judge))) %>%
  round(digits = 2)

# high-income & no-loss gambling : attractive rating
df3_spss %>%
  filter(split == 1, smallloss == 0) %>%
  summarise(meanJudgeHighIncomenoLossGamble = mean(judge), 
            CI.low = mean(judge) - (1.96 * Sem(judge)), 
            CI.high = mean(judge) + (1.96 * Sem(judge))) %>%
  round(digits = 2)
```
...
df3_spss %>%
  ...
  summarise(meanJudgeLowIncomeLossGamble = mean(judge), 
            CI.low = mean(judge) - (1.96 * Sem(judge)), 
            CI.high = mean(judge) + (1.96 * Sem(judge))) %>%
  round(digits = 2)
...
df3_spss %>%
  ...
  summarise(meanJudgeLowIncomeNoLossGamble = mean(judge), 
            CI.low = mean(judge) - (1.96 * Sem(judge)), 
            CI.high = mean(judge) + (1.96 * Sem(judge))) %>%
  round(digits = 2)
```
...
title <- factor(c("Lower Income", "Higher Income"), 
                levels = c("Lower Income", "Higher Income"))
values <- c(df3_low_loss$meanJudgeLowIncomeLossGamble, 
            df3_high_loss$mean.judgeHighIncomeLossGamble, 
            df3_low_noloss$meanJudgeLowIncomeNoLossGamble, 
            df3_high_noloss$meanJudgeHighIncomeNoLossGamble)
gamble <- c("loss_gamble", 
            "loss_gamble", 
            "no_loss_gamble", 
            "no_loss_gamble")
CIs <- c(df3_low_loss$CI, 
         df3_high_loss$CI, 
         df3_low_noloss$CI, 
         df3_high_noloss$CI)
...
# Change referenced :
fig3.table$gamble <- factor(fig3.table$gamble, 
                            levels = c("loss_gamble", "no_loss_gamble"))
figure3 <- fig3.table %>%
  ggplot(aes(x = title, 
             y = values, 
             fill = gamble)) + 
  geom_bar(stat = "identity", 
           position = position_dodge(), 
           colour = "black") + 
  ylab("Attractiveness of gamble") + 
  xlab(NULL) + 
  geom_errorbar(aes(ymin = values - CIs,
                    ymax = values + CIs), 
                width = 0.2, 
                position = position_dodge(0.9)) + 
  ggtitle("Figure 3 - Attractiveness of loss or no-loss gamble\nfor Study 3\n(error bars represent 95% confidence interval)") + 
  theme_bw() + 
  theme(panel.border = element_blank(), 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
...
# Partial eta squared : 
etaSquared(aov(judge ~ split * smallloss, df3_spss)) %>%
  .["split:smallloss", "eta.sq.part"] %>%
  round(digits=3) %>%
  cat("Low-/High-Income & Loss-/No-loss-gamble\npartial eta squared:",.)
```
...
title <- factor(c("Lower Income", "Higher Income"), 
                levels = c("Lower Income", "Higher Income"))
values <- c(df4_low_small$mean.expense.lowincome.smallaccount, 
            df4_high_small$mean.expense.highincome.smallaccount,
            df4_low_large$mean.expense.lowincome.largeaccount, 
            df4_high_large$mean.expense.highincome.largeaccount)
account_size <- c("small_account", 
                  "small_account", 
                  "large_account", 
                  "large_account")
CIs <- c(df4_low_small$CI, 
         df4_high_small$CI, 
         df4_low_large$CI, 
         df4_high_large$CI)
...
# change referenced level
fig4.table$account_size <- factor(fig4.table$account_size, 
                                  levels = c("small_account", 
                                             "large_account"))
figure4 <- fig4.table %>%
  ggplot(aes(x = title, 
             y = values, 
  ...
  geom_errorbar(aes(ymin = values - CIs, 
                ...
                width = 0.2, 
                position = position_dodge(0.9)) + 
  ggtitle("Figure 4 - DVD Expense Rating for either\na small or large account size for Study 4\n(error bars represent 95% confidence interval)") + theme_bw() + 
  theme(panel.border = element_blank(), 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
        axis.line = element_line(colour = "black"))
...
# partial eta squared :
etaSquared(aov(Expense ~ Large.Acc*Split, df4_1)) %>%
  .["Large.Acc:Split","eta.sq.part"] %>%
  round(digit=3) %>%
  cat("Small-/Large-account & Low-/High-Income\nPartial Eta-squared:",.)
```
...
```{r echo = FALSE}
# regression:
lm(scale(expense) ~ scale(largeacc) + 
     scale(logses) + 
     scale(largeacc * logses), 
   data = df4_spss) %>%
  tidy() %>%
...
df5_spss %>%
  ...
  summarise(mean.fattening.nondieters.smallaccount = mean(fattening), 
            CI.low = mean(fattening) - (1.96 * Sem(fattening)), 
            CI.high = mean(fattening) + (1.96 * Sem(fattening))) %>%
  round(digits = 2)

# Nondieters & Large account : Fattening
df5_spss %>%
  filter(split == 0, week == 1) %>%
  summarise(mean.fattening.nondieters.largeaccount = mean(fattening), 
            CI.low = mean(fattening) - (1.96 * Sem(fattening)), 
            CI.high = mean(fattening) + 
              (1.96 * Sem(fattening))) %>%
  round(digits = 2)
```
...
df5_spss %>%
  ...
  summarise(mean.fattening.dieters.smallaccount = mean(fattening), 
            CI.low = mean(fattening) - (1.96 * Sem(fattening)), 
            CI.high = mean(fattening) + (1.96 * Sem(fattening))) %>%
  round(digits = 2)
...
df5_spss %>%
  ...
  summarise(mean.fattening.dieters.largeaccount = mean(fattening), 
            CI.low = mean(fattening) - (1.96 * Sem(fattening)), 
            CI.high = mean(fattening) + (1.96 * Sem(fattening))) %>%
  round(digits = 2)
```
...
```{r echo = FALSE}
# Plotting : 
title <- factor(c("Dieting", "Not Dieting"), 
                levels = c("Dieting", "Not Dieting"))
values <- c(df5_diet_small$mean.fattening.dieters.smallaccount, 
            df5_nondiet_small$mean.fattening.nondieters.smallaccount,
            df5_diet_large$mean.fattening.dieters.largeaccount, 
            df5_nondiet_large$mean.fattening.nondieters.largeaccount)
account_size <- c("small_account", 
                  "small_account", 
                  "large_account", 
                  "large_account")
CIs <- c(df5_diet_small$CI, 
         df5_nondiet_small$CI, 
         df5_diet_large$CI, 
         df5_nondiet_large$CI)
# Figure data frame
fig5.table <- data.frame(title, values, account_size, CIs)
# Change reference level
fig5.table$account_size <- factor(fig5.table$account_size, 
                                  levels = c("small_account", 
                                             "large_account"))
figure5 <- fig5.table %>%
  ggplot(aes(x = title, 
  ...
  geom_bar(stat = "identity", 
           position = position_dodge(), 
           colour = "black") + 
  ylab("Fattiness rating") + 
  xlab(NULL) + 
  geom_errorbar(aes(ymin = values - CIs, 
                    ymax = values + CIs), 
                width = 0.2, 
                position = position_dodge(0.9)) + 
  ggtitle("Figure 5 - French Fry Fattiness rating for either\na small or large calorie account size for Study 5\n(error bars represent 95% confidence interval)") + 
  theme_bw() + 
  theme(panel.border = element_blank(), 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
...
# Partial Eta Squared
etaSquared(aov(fattening ~ split * week, df5_spss)) %>%
  .["split:week","eta.sq.part"] %>%
  round(digit=3) %>%
  cat("Small-/Large-Account & Not-/Is-Dieting\nPartial eta squared:",.)
```
...
df6_spss %>%
  ...
  summarise(mean.expense.timerich.smallaccount = mean(expense), 
            CI.low = mean(expense) - (1.96 * Sem(expense)), 
            CI.high = mean(expense) + (1.96 * Sem(expense))) %>%
  round(digits = 2)

df6_spss %>%
  filter(slack == 1, large == 1) %>%
  summarise(mean.expense.timerich.largeaccount = mean(expense), 
            CI.low = mean(expense) - (1.96 * Sem(expense)), 
            CI.high = mean(expense) + (1.96 * Sem(expense))) %>%
  round(digits = 2)
```
...
df6_spss %>%
  ...
  summarise(mean.expense.timepoor.smallaccount = mean(expense), 
            CI.low = mean(expense) - (1.96 * Sem(expense)), 
            CI.high = mean(expense) + (1.96 * Sem(expense))) %>%
  round(digits = 2)
...
df6_spss %>%
  ...
  summarise(mean.expense.timepoor.largeaccount = mean(expense), 
            CI.low = mean(expense) - (1.96 * Sem(expense)), 
            CI.high = mean(expense) + (1.96 * Sem(expense))) %>%
  round(digits = 2)
```
...
```{r echo = FALSE}
# Plotting
title <- factor(c("Time Poor", "Time Rich"), 
                levels = c("Time Poor", "Time Rich"))
values <- c(df6_poor_small$mean.expense.timepoor.smallaccount, 
            df6_rich_small$mean.expense.timerich.smallaccount, 
            df6_poor_large$mean.expense.timepoor.largeaccount, 
            df6_rich_large$mean.expense.timerich.largeaccount)
account_size <- c("small_account", 
                  "small_account", 
                  "large_account", 
                  "large_account")
CIs <- c(df6_poor_small$CI, 
         df6_rich_small$CI, 
         df6_poor_large$CI, 
         df6_rich_large$CI)
# figure 6 data frame:
...
fig6.table$account_size <- factor(fig6.table$account_size, 
                                  levels = c("small_account", 
                                             "large_account"))
figure6 <- fig6.table %>%
  ggplot(aes(x = title, 
  ...
  geom_errorbar(aes(ymin = values - CIs, 
                ...
                width = 0.2, 
                position = position_dodge(0.9)) + 
  ggtitle("Figure 6 - Time loss Expense rating for either\na small or large time account size for Study 6\n(error bars represent 95% confidence interval)") + 
  theme_bw() + 
  theme(panel.border = element_blank(), 
        panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), 
...
# partial eta squared
etaSquared(aov(expense ~ large * slack, df6_spss)) %>%
  .["large:slack","eta.sq.part"] %>%
  round(digit=3) %>%
  cat("Small-/Large-Account & Time-Poor/-Rich\nPartial eta squared:", .)
```
